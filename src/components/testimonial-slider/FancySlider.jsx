 ...
 *
 * Ported from the "bti" real-estate theme's own slider (it re-implements the
 * slice of Swiper it needs and ships nothing else). Behaviour preserved:
 *
 ...
 *               (fade: no transform; CSS cross-fades the slides)
 *   autoplay  = setInterval(next, delay); restarted after interaction when
 *               disableOnInteraction is false
 *   drag      = 8px axis lock; on release steps by round(-dx / slideWidth),
 *               or +/-1 once |dx| passes min(90, max(35, .18 * slideWidth))
 *
 * Ref API: slidePrev() slideNext() slideTo(i, instant) realIndex update() getRoot()
 *
 * Requires FancySlider.css. Children are the slides (any element); the engine
 * wraps nothing — pass whatever you want per slide.
 */
...
const FancySlider = forwardRef(function FancySlider(
  {
    id,
    options = {},
  ...
  const applyTransform = useCallback(
    (instant, explicit) => {
      ...
      if (instant) {
        // force reflow so the next transition takes
  ...
  const applyClasses = useCallback(() => {
    ...
    if (!fade) {
      for (
        let i = st.index;
        i < st.index + st.perView && i < slides.length;
        i++
      ) {
